 ...
 *   因为每粒沙的左邻居总是先被处理。它还有个更隐蔽的效果 ——
 *   扫描顺序决定了**消息能在一帧之内传出去多远**：顺扫描方向，一格让位、
 *   下一格当场跟进，可以一路连锁到画布另一端；逆方向每帧只推进一格。
 *   实测差十倍。逐帧翻转把两侧扯平，代价是两侧都变快。
 *
...
export class SandWorld {
  ...
  private tryMove(
    ...
    // 横着挪时，不许占用**本帧刚刚腾出来的**空格。
    //
    // 少了这一句，一格填进缺口、让出自己的位置，而它下一格本帧还没轮到，
    // 当场就跟进来 —— 一路连锁到画布另一端，全在同一帧里。后果是从水池
    // 中间舀一勺水，缺口一帧就被填满，而账记在扫描方向那一侧的几格上，
    // 既不对称也不像漏斗。挡住之后，缺口每帧只能从紧挨着的一格补，
    // 凹陷才会真的一圈圈向外摊开。
    //
    // 只挡水平方向。斜下那一路挡不得，试过：
    //
    // 一竖列水被舀走后，两边的水是**斜着**滑进去的，所以连锁照样成立，
    // 把斜下也挡上，凹陷的形状确实对了（对称的局部凹坑，而不是「中间填满、
    // 账记在一侧」）—— 但斜下同时是找平的主干道，掐掉它水就不再找平了：
    // 同一个水池跑四千帧，各列水深还差七格（只挡水平时差一格）。
    // 竖直方向更不能挡，那正是「一柱沙整体下落」的来源。
    if (ty === y && this.mat[j] === EMPTY && this.clock[j] === this.frame) {
      return false;
    }
