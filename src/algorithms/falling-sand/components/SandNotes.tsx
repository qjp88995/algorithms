  ...
  return (
    <>
      ...
      <NoteSection title="四条运动规则，就这些">
        ...
        <p>
          液体那一行最容易写错的是：
          <span className="text-ink">横着挪必须有个去处</span>。
          「只要旁边空着就挪几格」看着很自然，可同一高度上的平移什么都没 改变 ——
          高度没降、低洼没填。后果是孤零零浮在水面上的一格水每帧
          随机往一边弹几格，把水面搅成一排周期恰好等于铺开距离的锯齿，
          而且停不下来：形状不再变化，水却一直在跳。
        </p>
        <p>
          所以横向流动只在两种情况下发生：视野内找得到一个
          <span className="text-ink">底下是空的</span>
          位置（那才是往低处走），或者头上还压着同类液体（那是压力驱动的
          铺开）。剩下的一律待着不动。
        </p>
      </NoteSection>
