    ...
    <>
      ...
      <NoteSection title="扫描顺序本身就是规则">
        ...
        <p>
          ...
          。 固定从左往右扫时，在水面上挖个洞 —— 洞右边那格水填进去、腾出位置，
          而它右边那格<span className="text-ink">本帧还没轮到</span>
          ，于是当场跟着填进来，再让位给下一格……一路连锁到画布最右端，
          全发生在同一帧里。洞左边正相反：那些格子早在洞被填之前就处理完了，
          每帧只能往外推进一格。
        </p>
        <p>
          实测这个不对称差十倍：固定方向时，顺扫描方向一百四十格外第十三帧
          就降了，逆方向要等到第一百一十九帧。逐帧翻转把两边扯平， 代价是
          <span className="text-ink">两边都变快</span> ——
          所以擦掉中间一块水，左右几乎同时下沉。这是顺序扫描的固有伪影，
          要根除只能改成读旧写新的双缓冲，而那样就地交换的整套机制都没了，
          沙子也不会整柱下落。
        </p>
      </NoteSection>
      ...
      <NoteSection title="它的水是假的">
        ...
        <p>
          挖开一处水面，两百格外的水十几帧就开始跟着降 —— 六分之一秒，
          看着就像整片一起塌。但这不是因为水流得快，
          <span className="text-ink">一帧横着挪几格跟它没关系</span>：
          真正的原因是扫描顺序让消息在同一帧里连锁传了出去，
          详见「扫描顺序本身就是规则」那一节。
        </p>
        <p>
          想看清「最近的水先补上，再一圈圈传出去」，
